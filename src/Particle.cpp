...
double Particle::vision_weight(LikelihoodFieldMap *map, Scan &scan, const int &valid_beams, const bool use_vision, const yolov5_pytorch_ros::BoundingBoxes& bbox, const YAML::Node& landmark_config, double prob_th, double phi_th, double R_th, double w_img, double ratio)
{
	uint16_t t = p_.get16bitRepresentation();
	double lidar_x = p_.x_ + scan.lidar_pose_x_*Mcl::cos_[t] 
				- scan.lidar_pose_y_*Mcl::sin_[t];
...
double Particle::vision_weight(LikelihoodFieldMap *map, Scan &scan, const int &valid_beams, const bool use_vision, const yolov5_pytorch_ros::BoundingBoxes& bbox, double prob_th, const std::vector<Inv_Det>& inv_det, double w_img, double ratio)
{
	uint16_t t = p_.get16bitRepresentation();
	double lidar_x = p_.x_ + scan.lidar_pose_x_*Mcl::cos_[t] 
				- scan.lidar_pose_y_*Mcl::sin_[t];
