...
void register_landmark::cb_yolo(const yolov5_pytorch_ros::BoundingBoxes& msg)
{
    ...
    for (const auto &b:msg.bounding_boxes)
    {
        ...
        if (it != landmark_list.end() && cloud_.points.size() != 0 && b.probability > 0.9)
        {
            lm.class_ = b.Class;
...
void register_landmark::visualize_landmark(std::vector<landmark> &lm_list)
{
    ...
    {
        text_.text = lm.class_.c_str();
        sphere_.pose.position.x = lm.pos_[0];
        sphere_.pose.position.y = lm.pos_[1];
        sphere_.pose.position.z = lm.pos_[2];
        text_.pose.position.x = lm.pos_[0];
        text_.pose.position.y = lm.pos_[1] + 0.4;
        text_.pose.position.z = lm.pos_[2];
        sphere_.color.r = 0.0;
        sphere_.color.g = 0.0;
        sphere_.color.b = 1.0;
        sphere_.color.a = 0.8;
