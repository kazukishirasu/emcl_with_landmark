...
class register_landmark{
    private:
        ...
        tf::MessageFilter<sensor_msgs::LaserScan> scan_notifier;
    public:
        register_landmark(ros::NodeHandle n);
...
register_landmark::register_landmark(ros::NodeHandle n) :
        nh(n),
        scan_sub(nh, "/scan", 10),
        scan_notifier(scan_sub, listener, "map", 10){
    ROS_INFO("Start register_landmark node");
    // scan_sub = nh.subscribe("/scan", 1, &register_landmark::scan_callback, this);
    scan_notifier.registerCallback(boost::bind(&register_landmark::scan_callback, this, _1));
    scan_notifier.setTolerance(ros::Duration(0.01));
